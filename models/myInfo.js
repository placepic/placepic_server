...
const myInfo = {
    ...
    getPlacesWithUser : async(userIdx,groupIdx) => {
        const getPlacesWithUser =  `SELECT placeIdx,placeImageUrl,placeName FROM (SELECT placeIdx,placeImageUrl,placeName, groupIdx, userIdx FROM PLACE_TB as p natural left outer join PLACEIMAGE_TB as i where p.placeIdx = i.placeIdx)as a WHERE a.groupIdx = 1 and userIdx = 90 group by placeIdx`
